// /pages/upload-history-signed-filter-size.tsx
...
export default function UploadHistorySignedFilterSizePage() {
  ...
  const fetchResults = async () => {
    ...
    if (data) {
      const withSizes: ResultItem[] = await Promise.all(
        data.map(async (item) => {
          const { data: statData, error: statError } = await supabase.storage
            .from('results')
            .getPublicUrl(item.file_path);

          // Supabase JS SDK には直接サイズ取得APIがないため、HeadリクエストかDBにサイズを保存するのが推奨です。
          // 今回はサイズをnullとして、必要なら別途保存かStorageリスト取得API使用をおすすめします。

          return { ...item, size: null }; // サイズ取得を省略する場合
        })
      );

      setAllResults(withSizes);
      setFilteredResults(withSizes);
    }
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">署名付きURL履歴リスト（サイズ表示付き）</h1>
