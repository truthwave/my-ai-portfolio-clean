// /pages/upload-multi-with-size.tsx
...
export default function UploadMultiWithSizePage() {
  ...
  const handleUpload = async () => {
    ...
    for (const file of files) {
      ...
      uploaded.push({ id: storageData?.Key || filePath, file_path: filePath, url, size });
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6">
      <h1 className="text-2xl font-bold mb-4">複数画像アップロード・サイズ記録・削除</h1>
      ...
      <div className="mt-6 grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
        {uploadedFiles.map((file) => (
          <div key={file.file_path} className="border rounded p-2 flex flex-col items-center">
            <p className="text-xs text-gray-700 mb-1">サイズ: {formatSize(file.size)}</p>
